...
num_gender_class = len(pd.value_counts(df_all['Gender']))
# print(num_gender_class)  # 3
n = 5
...
stack = np.zeros((x.shape[0], num_gender_class))
# print(stack.shape)
stack_te = np.zeros((x_te.shape[0], num_gender_class))
...
score_va /= n
score_te /= n
print('验证集性别的平均准确率:%f' % score_va)
print('测试集性别的平均准确率:%f' % score_te)
...
df_stack.to_csv(config.feat_path + '/tfidf/lr_prob_gender.csv', index=False, encoding='utf-8')
# 交叉验证
...
df_stack.to_csv(config.feat_path + '/tfidf/mnb_prob_gender.csv', index=False, encoding='utf-8')
...
df_stack.to_csv(config.feat_path + '/tfidf/svm_prob_gender.csv', index=False, encoding='utf-8')
