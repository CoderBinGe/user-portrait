...
# 加载特征
df_tfidf_lr = pd.read_csv(config.feat_path + '/tfidf/lr_prob_gender.csv')
df_tfidf_mnb = pd.read_csv(config.feat_path + '/tfidf/mnb_prob_gender.csv')
df_tfidf_svm = pd.read_csv(config.feat_path + '/tfidf/svm_prob_gender.csv')
df_w2v = pd.read_csv(config.feat_path + '/w2v/w2v_avg.csv')

# 加载标签
df_lb = pd.read_csv(config.corpus_file, usecols=['ID', 'Gender'], encoding='utf-8',
...
# print(df.columns)
x = df.iloc[:tr_num]
y = df_lb['Gender'][:tr_num]
x_te = df.iloc[tr_num:]
y_te = df_lb['Gender'][tr_num:]

df_sub = pd.DataFrame()
df_sub['ID'] = df_lb.iloc[tr_num:]['ID']

# 参数设置
num_gender_class = len(pd.value_counts(df_lb['Gender']))
params = {
    'booster': 'gbtree',
    'objective': 'multi:softmax',  # 多分类
    'stratified': True,
    'num_class': num_gender_class,  # 类别数
...
print('测试集性别的准确率: %.3f' % accuracy)

df_sub['Gender'] = (bst.predict(dtest)).astype(int)
df_sub['ID'] = df_sub['ID'].astype(str)
df_sub.to_csv(config.output + '/gender_result.csv', encoding='utf-8', index=False)
